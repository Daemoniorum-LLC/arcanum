...
Arcanum does not compete with RustCrypto. Our native primitives serve specific optimization goals (HCT decompression, batch hashing for model weights). The dual-backend approach lets users make informed decisions about their risk tolerance.
...
|---------|-------------|----------|
| `backend-native` | Arcanum's native primitives | Optimized for inference workloads, smaller bundles |
...
2. **Not Competing**: Arcanum does not seek to replace or compete with established libraries like RustCrypto. Our native primitives exist for specific optimization goals (batch operations, inference workloads, HCT integration). The RustCrypto backend exists precisely because those libraries are battle-tested.

3. **Informed Choice**: By exposing both backends, users can make their own risk/performance tradeoffs. Security-critical applications should use `backend-rustcrypto`. Performance-critical inference pipelines may choose `backend-native`.
...
# For native Arcanum primitives (inference-optimized, not audited)
